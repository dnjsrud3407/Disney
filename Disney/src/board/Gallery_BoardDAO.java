...
public class Gallery_BoardDAO {
	...
	public String compareDate(Gallery_BoardBean getbb) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		try {
			con = getConnection();
			sql = "select time(date) from gallery where date(now())=date(?) and num=?";
			pstmt = con.prepareStatement(sql);
			...
			if (rs.next()) {
				...
				sql = "select date(date) from gallery where num=?";
				pstmt = con.prepareStatement(sql);
	...
	public List getBoardList(int startRow, int pageSize) {	
		...
		try {
			...
			while(rs.next()) {
				...
				bb.setCmtCount(rs.getInt("cmtCount"));
				list.add(bb);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List getBoardList(int startRow, int pageSize, String search) {	
		...
		try {
			con = getConnection();
			String sql = 
			"select * from gallery where subject like ? order by num desc limit ?,?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, "%" + search + "%");
	...
	public Gallery_BoardBean getBoard(int num) {	// content에서 게시글 보기
		Gallery_BoardBean bb = new Gallery_BoardBean();
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public int insertBoard(Gallery_BoardBean getbb) {	//writeBoard에서 게시글 작성
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int num = 0;
		try {
			...
			sql = "insert into gallery(id, subject, content, readcount, date, num, file, cmtCount)"
				+ "values (?,?,?,?,now(),?,?,0)";
			pstmt = con.prepareStatement(sql);
	...
	public void updateForm(Gallery_BoardBean bb) {
		Connection con = null;
		PreparedStatement pstmt = null;
		try {
			con = getConnection();
			String sql = "update gallery set subject=?, content=?, file=? where num=?";
			pstmt = con.prepareStatement(sql);
